...
func main() {
	...
	if err := pm.RegisterInternalSigningHandler(signingHandler); err != nil {
		setupLog.Error(err, "failed to register internal signing plugin")
		os.Exit(1)
	}

	if err := pm.RegisterInternalCredentialRepositoryPlugin(
	...
	// No Helm input plugin is registered here; declare Helm credential types explicitly.
